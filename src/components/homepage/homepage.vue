    ...
    <div class="section_one">
      <div class="section_one_bottom">
        <div class="section_one_bottom_img locationR">
          ...
          <i @click="deleteLogo" style="z-index:10000;" class="icon iconfont icon-shanchu1 locationA"></i>
      ...
      <ul>
        <li class="lef locationR" v-for="(item,$index) in listImgDataObj">
          ...
          <div v-bind:id="uploadContainerObj[$index]" class="commonUploadContainer">
            ...
            <a v-bind:id="selectfilesBtnObj[$index]" href="javascript:void(0);" class='btn commonBtnRight '>更换</a>
          </div>
          <i class="icon iconfont icon-shanchu1 locationA" @click="deleteListPic($index,item.pic.file_path)" style="top:35px;"></i>
        </li>
      </ul>
      <p class="introduceP">注：可以修改模块名称，模块图尺寸为340*520，请按照尺寸上传图片，其他尺寸会影响显示效果。图片格式只能是jpg或者png,单张图片大小不要过2M，删除图片点击模块右上角"X"</p>
    </div>
...
<script>
    ...
    export default {
        ...
        created:function(){
          ...
          this.getCustomizedData();//获取定制页面数据
        },
        mounted:function(){
          var self = this;
          //logo上传
        ...
        methods:{
          ...
          deleteBackgroundPic:function(){
            ...
            if(!!that.backgroundPic){
              ...
              // }).then(() => {
              //   var data = {};
              //   data['file_id'] = that.backgroundImgDataObj['8'].pic.file_id;
              //   that.deletePicFunc(data);
              // }).catch(() => {
              //   that.$message({
              //     type: 'info',
          ...
          getCustomizedData:function(data){
            ...
            PT.apiClient.query(SvrApi.customer, '',  function(rsp) {
              ...
              if(newData.code == 0){
                 ...
                 //拆分logo,横版图,背景图
                 self.listImgDataObj = func.copyObj1(self.allData.type);//列表
                 self.logoImgDataObj = func.copyObj2(self.allData.type);//logo
                 self.backgroundImgDataObj = func.copyObj3(self.allData.type);//背景图
